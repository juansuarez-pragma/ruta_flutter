...
- **Dependency injection:** `lib/src/core/injection_container.dart` uses `get_it` (service-locator). Register or replace bindings here. `EnvConfig` must be initialized before calling `init()`.
...
- **Naming & structure conventions (observable):**
  ...
  - Models are immutable and provide `fromJson`, `toJson`, and `toEntity` methods (see `lib/src/data/models/product_model.dart`).
...
- **Key files to inspect for context:**
  - `lib/src/core/injection_container.dart`
