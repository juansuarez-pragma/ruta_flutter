...
void main() {
  ...
  group('ProductRepositoryImpl', () {
    group('getAllProducts', () {
      test('retorna Right con lista de productos cuando datasource tiene éxito', () async {
        // Arrange
        final testModels = createTestProductModelList(count: 3);
        when(mockProductDataSource.getAll())
    ...
    group('getProductById', () {
      ...
      test('retorna Right con producto cuando datasource tiene éxito', () async {
        // Arrange
        final testModel = createTestProductModel(id: testId);
        when(mockProductDataSource.getById(testId))
      ...
      test('retorna Left NotFoundFailure cuando datasource lanza NotFoundException', () async {
        // Arrange
        when(mockProductDataSource.getById(testId))
      ...
      test('pasa el ID correcto al datasource', () async {
        ...
        when(mockProductDataSource.getById(specificId))
            .thenAnswer((_) async => testModel);
    ...
    group('getAllCategories', () {
      test('retorna Right con lista de categorías cuando datasource tiene éxito', () async {
        // Arrange
        final testCategories = createTestCategories();
        when(mockCategoryDataSource.getAll())
      ...
      test('retorna Left NotFoundFailure con mensaje personalizado cuando datasource lanza NotFoundException', () async {
        // Arrange
        ...
        result.fold(
          (failure) {
            expect(failure, isA<NotFoundFailure>());
            expect(failure.message, AppStrings.notFoundCategoriesFailureMessage);
          },
          (_) => fail('No debería retornar Right'),
        );
      });
